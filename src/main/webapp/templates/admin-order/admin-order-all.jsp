...
<div>
    <table id="order-table-all" class="stripe">
        <thead>
        <tr>
            <th>ID</th>
            <th>Image</th>
            ...
            <tr>
                ...
                <td class="order-title">
                    <c:choose>
                        <c:when test="${Order.getOrderShoeStatus() == 'pending'}">
                            <p class="order-tag" style="background-color: #adadad">Pending</p>
                        </c:when>
                ...
                <td>
                    <button type="button" class="btn btn-primary" data-bs-toggle="modal"
                            data-bs-target="#modal-status-${Order.getId()}">
                        Change
                    </button>

                    <div class="modal fade" id="modal-status-${Order.getId()}" tabindex="-1"
                        ...
                        <div class="modal-dialog modal-dialog-centered">
                            <div class="modal-content">
                                <form method="POST"
                                    ...
                                    <input type="hidden" name="email" value="${Order.getUser().getEmail()}">
                                    <input type="hidden" name="nameCustomer" value="${Order.getUser().getFirstname()}">
                                    <input type="hidden" name="title" value="${Order.getShoe().getTitle()}">
                                    <input type="hidden" name="size" value="${Order.getSize().getSizeName()}">
                                    <input type="hidden" name="price" value="${Order.getShoe().getPrice()}">
                                    <input type="hidden" name="price" value="${Order.getShoe().getPrice()}">
                ...
                <td>
                    <button type="button" class="btn btn-primary" data-bs-toggle="modal"
                            data-bs-target="#modal-all-info-${Order.getId()}">
                        Info
                    </button>
                    <div class="modal fade" id="modal-all-info-${Order.getId()}" tabindex="-1"
                         aria-labelledby="exampleModalLabel" aria-hidden="true">
