...
@WebServlet(name = "api-order-texture", urlPatterns = {"/api/order-texture"})
public class ApiGetOrderTexture extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        Order order = OrderManager.get().getImageByOrderId(Integer.parseInt(idStr));
        if (order == null) {
            response.setStatus(HttpServletResponse.SC_NOT_FOUND);
            return;
        }

        byte[] imageBytes = order.getTextureImage();
